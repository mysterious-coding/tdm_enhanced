...
static void PM_SetVehicleAngles( vec3_t normal )
{
	...
	vehicleBankingSpeed = (pVeh->m_pVehicleInfo->bankingSpeed*32.0f)*pml.frametime;
	...
	{//wall-running up
		...
		{//still a vertical wall there
			...
			{//all clear, keep going
				...
				//test PMOVE FIX remove if making problems
				return qtrue;
			}
		}
		//failed!
		...
		{//can't do this in zero-G
			...
			{//holding jump in air                           
				...
				{//can still go up
					...
					pm->ps->velocity[2] = (forceJumpHeight[pm->ps->fd.forcePowerLevel[FP_LEVITATION]]-curHeight)/forceJumpHeight[pm->ps->fd.forcePowerLevel[FP_LEVITATION]]*forceJumpStrength[pm->ps->fd.forcePowerLevel[FP_LEVITATION]];
					pm->ps->velocity[2] /= 10;
				...
				if ( allowWallRuns )
				{
					...
					if (1) //sure, we have it! Because I SAID SO.
					{
						...
						{//there is a wall there
							...
							//test PMOVE FIX - return if any problems occur
						}
					}
				}
			}
...
static void PM_CrashLandEffect( void )
{
	...
	delta = fabs(pml.previous_velocity[2])/10;
	if ( delta >= 30 ) 
	{
		vec3_t bottom;
		int	effectID = -1;
...
static void PM_CheckDuck (void)
{
	...
	else
	{
		...
		{
			//Xycaleths fix for crouch jumping through some surfaces
            if ( PM_CanStand() )
            {
                pm->maxs[2] = pm->ps->standheight;
                pm->ps->pm_flags &= ~PMF_ROLLING;
		...
		{	// stand up if possible 
			if (pm->ps->pm_flags & PMF_DUCKED)
			{
				//Xycaleths fix for crouch jumping through surfaces part 2
                if ( PM_CanStand() )
	            {
		            pm->maxs[2] = pm->ps->standheight;
		            pm->ps->pm_flags &= ~PMF_DUCKED;
...
int PM_LegsSlopeBackTransition(int desiredAnim)
{
	...
	case LEGS_S5_RUP5:
		if (pm->ps->slopeRecalcTime < pm->cmd.serverTime)
		{
			...
			pm->ps->slopeRecalcTime = pm->cmd.serverTime + 8;
		}
		else
		{
			resultingAnim = anim;
...
static void PM_Footsteps( void ) {
	...
	else if ( pm->ps->pm_flags & PMF_DUCKED )
	{
		...
		if ( ( (PM_RunningAnim( pm->ps->legsAnim )&&VectorLengthSquared(pm->ps->velocity)>=10000/*100*100*/) || PM_CanRollFromSoulCal( pm->ps ) ) &&
...
static void PM_Weapon( void )
{
	...
	{
		if (!pm->ps->duelInProgress || g_gametype.integer != GT_CTF ){
			pm->cmd.weapon = WP_SABER;
			pm->ps->weapon = WP_SABER;
					 ...
					 vec3_t origin, vec3_t angles, vec3_t scale, int blendTime, qboolean forceHalt)
{
	...
	{ //actively update our ik state.
		...
		{
			ikM.movementSpeed = 0.9f;
		}
		else if (distToDest < 32)
		{
			ikM.movementSpeed = 0.8f;
		}
		else if (distToDest < 64)
		{
			ikM.movementSpeed = 0.7f;
		}
		else
		{
